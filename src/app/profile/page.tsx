...
const profile = async () => {
    ...
    const profile = data?.profile || {
        name: 'Dukuh',
        deskripsi: 'Data profil belum tersedia.',
        total_penduduk: 0,
        total_umkm: 0,
            ...
            <article className="w-full min-h-screen flex flex-col p-4">
                <nav className="bg-white flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-lg shadow-lg border border-gray-200 sticky top-5 z-50">
                    <section className="flex gap-4">
                        ...
                        <h1 className="text-xl sm:text-3xl text-primary font-bold"><Link href="/">Profil {profile.name}</Link></h1>
                    </section>
                ...
                <article className="max-h-screen overflow-y-scroll flex flex-col md:flex-row gap-4 p-4 mt-4 rounded-lg">
                    <Card className="w-full md:w-2/3">
                        <CardHeader>
                            <CardTitle className="text-2xl font-bold text-primary">Profil {profile.name}</CardTitle>
                        </CardHeader>
                        <CardContent className="flex flex-col justify-center gap-4 items-center w-full">
                            <Carousel className="max-w-sm md:max-w-5xl" opts={
                                {
                                    loop: true
                                }
                            }>
                                <CarouselContent>
                                    {images.map((image, index) => (
                                        <CarouselItem key={index} className="w-full flex justify-center">
                                            <Image
                                                src={image?.image_url ?? "/placeholder.webp"}
                                                alt={`Profile Image ${index + 1}`}
                                                width={800}
                                                height={450}
                                                className="rounded-md aspect-video object-cover w-full h-60 md:h-96"
                                            />
                                        </CarouselItem>
                                    ))}
                                </CarouselContent>
                        ...
                        <Card className="h-full">
                            <CardContent className="flex gap-4 justify-between items-center">
                                <section className="flex flex-col gap-2 justify-between">
                                    ...
                                    <p className="text-4xl md:text-6xl font-extrabold text-secondary">{profile.total_penduduk}</p>
                                </section>
                        ...
                        <Card className="h-full">
                            <CardContent className="flex gap-4 justify-between items-center">
                                <section className="flex flex-col gap-2 justify-between">
                                    ...
                                    <p className="text-4xl md:text-6xl font-extrabold  text-secondary">{profile.total_umkm}</p>
                                </section>
                        ...
                        <Card className="h-full">
                            <CardContent className="flex gap-4 justify-between items-center">
                                <section className="flex flex-col gap-2 justify-between">
                                    ...
                                    <p className="text-4xl md:text-6xl font-extrabold  text-secondary">{profile.total_fasilitas_umum}</p>
                                </section>
                ...
                <article className="max-h-screen overflow-y-scroll flex flex-col md:flex-row gap-4 p-4 mt-4 rounded-lg">
                    ...
                    <article className="flex-1 flex gap-4 flex-col">
                        <Card className="size-full">
                            ...
                            <CardContent className="flex gap-4 justify-between items-center">
                                <Link href={`https://wa.me/${formatPhoneForWhatsApp(profile.kontak)}`} className="w-full py-2 px-4 duration-500  hover:bg-gray-100 rounded-lg flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" width="24" height="24" viewBox="0 0 1024 1024" id="whatsapp">
                                    <defs>
                                ...
                                </svg><section className="w-full flex gap-2 justify-between text-sm md:text-base"><p>{formatPhoneDisplay(profile.kontak)}</p><span className="font-semibold">({profile.nama_kontak})</span></section></Link>
                            </CardContent>
                        </Card>
                        <Card className="size-full">
                            <CardContent className="flex gap-4 justify-between items-center">
                                <Button variant="ghost" className="size-full text-primary font-semibold" asChild>
                                    <Link href="https://instagram.com/dukuh_ngabean" target="_blank" className="flex items-center gap-2 p-4">
                                    ...
                                    <p className="font-bold text-primary text-nowrap text-base md:text-2xl">@dukuh_ngabean</p>
                                    </Link>
                                </Button>
                            </CardContent>
                        </Card>
